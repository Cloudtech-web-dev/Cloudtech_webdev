...
export const RotatingLogo = ({
	...
	const
		...
		logoCenterDelta = (size - (sizesRatio * diameter)) / 2,
		logoScaleCoefficient = sizesRatio * radius / originalHalf
	;
	
	return (

		<div className={className} style={{ color: "var(--bs-secondary-2)", ...externalStyle }}>
			<svg viewBox={`0 0 ${size} ${size}`} xmlns="http://www.w3.org/2000/svg" style={{ width: '100%' }} {...svgProps}>
				...
				<g>
					...
					<text fill="currentColor" fontFamily="system-ui, sans-serif" fontSize="9.5" fontWeight="600">
						<textPath href="#text-circle" textLength={circumference - 2} alignmentBaseline="before-edge">
							{textProp} · {textProp} ·&nbsp;
						</textPath>
					</text>
				</g>
				...
				<g transform={`
					translate( ${logoCenterDelta + 1} ${logoCenterDelta + 0.5} )
					scale( ${logoScaleCoefficient} )
				`}>
